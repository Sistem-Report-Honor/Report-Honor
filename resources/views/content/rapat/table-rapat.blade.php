        ...
        <div class="rounded-lg p-6 bg-[#EBE9EE] min-h-screen">
            <div id="rapat-sekarang" class="tab-content grid lg:grid-cols-4 gap-4">
                    ...
                    @foreach ($filteredRapats as $rapat)
                        <div class="bg-white rounded-lg p-4 shadow-sm shadow-indigo-200 w-full h-fit">
                            ...
                            <div class="mt-2">
                                ...
                                <div
                                    class="mt-4 flex grid lg:grid-cols-2 gap-6 text-xs justify-evenly bg-[#faf5ff] p-2.5 rounded-md">
                                ...
                                <div class="mt-4">
                                    @if ($rapat->status == 'prepare')
                                        <form id="start-meeting-form" action="{{ route('mulai', $rapat->id) }}"
                                            method="POST">
                                            @csrf
                                    ...
                                    @elseif ($rapat->status == 'mulai')
                                        <form id="end-meeting-form" action="{{ route('selesai', $rapat->id) }}"
                                            method="POST">
                                            @csrf
                                ...
                                <a href="{{ route('generate-pdf', $rapat->id) }}" target="_blank"
                                    class="inline-block bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mt-2">Print
                                    QR Code</a>
                            </div>
                        </div>
                    @endforeach
                @endif
                                ...
                                <a href="{{ route('print.qr', $rapat->id) }}" target="_blank"
                                    class="inline-block bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mt-2">Print
                                    QR Code</a>
                            </div>
                        </div>
                    ...
                    @foreach ($filteredRapats as $rapat)
                        <div class="bg-white rounded-lg p-4 shadow-sm shadow-indigo-200 w-fit h-fit">
                            ...
                            <div class="mt-2">
                                ...
                                <div
                                    class="mt-4 flex items-center gap-6 text-xs justify-evenly bg-[#faf5ff] p-2.5 rounded-md">
                                    <div class="sm:inline-flex sm:shrink-0 sm:items-center sm:gap-2">
                                        <i class='bx bx-calendar text-xl text-[#6e2bb1]'></i>
                                        <div class="mt-1.5 sm:mt-0">
                                            <p class="text-gray-500">Tanggal</p>
